...
{
    #[Route('/', name: 'examen')]
    public function examen(QuestionRepository $questionRepo , UserRepository $userRepo , EntityManagerInterface $em , Security $sec): Response
    {
    ...
    public function question( int $number  , EntityManagerInterface $em , int $passerExamenId , Security $sec): Response
    {
        if($sec->isGranted('IS_AUTHENTICATED_FULLY')){
    ...
    public function reponse(PasserExamenRepository $passerExamenRepo , int $passerExamenId , int $number , Security $sec): Response
    {
        if(!$sec->isGranted('IS_AUTHENTICATED_FULLY')){
            return $this->redirect($this->generateUrl('app_login'));
        }
